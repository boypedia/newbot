let toMs = require('ms')
const { smsg, fetchJson, getBuffer, fetchBuffer, getGroupAdmins, TelegraPh, msToDate, isUrl, hitungmundur, sleep, checkBandwidth, runtime } = require('./simple')
...
async function pagi(RiooXdzz, x, data) {
    ...
    return await RiooXdzz.sendMessage(x.room, {
        text: pagii(x),
        contextInfo: {
            externalAdReply: {
...
async function voting(RiooXdzz, x, data) {
    let row = [];
    let ment = [];
...
async function malam(RiooXdzz, x, data) {
...
async function skill(RiooXdzz, x, data) {
    skillOn(x.room, data)
        ...
        for (let i = 0; i < x.player.length; i++) {
            if (x.player[i].role === "werewolf") {
                if (x.player[i].isdead != true) {
                    ...
                    await RiooXdzz.sendMessage(x.player[i].id, {
                        text: textnya,
                        mentions: membernya,
                    });
                }
...
async function win(x, t, RiooXdzz, data) {
    ...
    if (getWinner(x.room, data).status === false || x.iswin === false) {
        ...
        return await RiooXdzz.sendMessage(sesinya, {
            text: textnya,
            contextInfo: {
                externalAdReply: {
                    title: "W E R E W O L F",
...
// playing
async function run(RiooXdzz, id, data) {
    while (getWinner(id, data).status === null) {
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, RiooXdzz, data);
            break;
        } else {
            await sleep(90000);
        }
        if (getWinner(id, data).status != null) {
            ...
            await pagi(RiooXdzz, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, RiooXdzz, data);
            break;
        ...
        if (getWinner(id, data).status != null) {
            ...
            await voting(RiooXdzz, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, RiooXdzz, data);
            break;
        ...
        if (getWinner(id, data).status != null) {
            ...
            await malam(RiooXdzz, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, RiooXdzz, data);
            break;
        } else {
            await skill(RiooXdzz, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) break;
    }
    await win(sesi(id, data), 1, RiooXdzz, data);
}

async function run_vote(RiooXdzz, id, data) {
    while (getWinner(id, data).status === null) {
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, RiooXdzz, data);
            break;
...
async function run_malam(RiooXdzz, id, data) {
    while (getWinner(id, data).status === null) {
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, RiooXdzz, data);
            break;
...
async function run_pagi(RiooXdzz, id, data) {
    while (getWinner(id, data).status === null) {
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, RiooXdzz, data);
            break;
