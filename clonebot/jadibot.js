...
const jadibot = async (rioo, m, from) => {
      ...
      client[from].ev.on("connection.update", async up => {
        ...
        if (connection === "close") {
          if (reason === DisconnectReason.restartRequired) {
            ...
            return rioo.sendMessage(from, { text: "Anda sudah tidak lagi menjadi bot." })
          }
        }
      })
      ...
      client[from].ev.process(async (events) => {
        if (events['messages.upsert']) {
          ...
          for (let msg of upsert.messages) {
            ...
            require("../batten")(client[from], m, store)
          }
        }
      })
...
async function stopjadibot(rioo, from) {
  if (!Object.keys(client).includes(from)) {
    return rioo.sendMessage(from, { text: "Anda tidak ada di list jadi bot." })
  }
  try {
...
async function listjadibot(rioo, m) {
  ...
  return rioo.sendMessage(from, { text: text.trim(), mentions, })
}
